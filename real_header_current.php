...
<style>
    ...
    /* Let Bootstrap handle dropdown visibility - just style appearance */
    .dropdown-menu {
        background: var(--surface);
    ...
    /* Bootstrap button reset for avatar */
    .btn.user-avatar {
        border: 2px solid var(--border-color);
        padding: 0;
    }
                ...
                <div class="dropdown" id="categoriesDropdown">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        try {
                            ...
                            if ($connection && !$connection->connect_error) {
                                ...
                                if ($resultCategories && mysqli_num_rows($resultCategories) > 0) {
                                    while ($category = mysqli_fetch_assoc($resultCategories)) {
                                        ...
                                        echo '<li><a href="/category/' . htmlspecialchars($category['url_category']) . '" class="dropdown-item" title="' . 
                                             htmlspecialchars($category['title_category']) . '">' . 
                                             htmlspecialchars($shortTitle) . '</a></li>';
                                        $hasCategories = true;
                                    }
                                }
                            }
                            ...
                            if (!$hasCategories) {
                                echo '<li><a href="#" class="dropdown-item disabled">Категории не найдены</a></li>';
                            }
                        } catch (Exception $e) {
                            echo '<li><a href="#" class="dropdown-item">Ошибка загрузки категорий</a></li>';
                        }
                        ?>
                    </ul>
                </div>
