...
try {
    ...
    // Now load email functions (which depends on constants.php)
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if ($insertStmt) {
                            ...
                            if ($insertStmt->execute()) {
                                ...
                                // Use PHPMailer directly since constants might not be loaded
                                ...
                                try {
                                    ...
                                    // Recipients
                                    $mail->setFrom($_ENV['ADMIN_EMAIL'] ?? 'noreply@11klassniki.ru', '11классники');
                                    $mail->addAddress($email);
                                    
                                    // Content
                                    $mail->isHTML(true);
                                    $mail->Subject = 'Восстановление пароля - 11классники';
                                    $mail->Body    = getPasswordResetEmailTemplate($user['firstname'], $resetLink);
                                    $mail->AltBody = strip_tags("Здравствуйте, {$user['firstname']}!\n\nДля восстановления пароля перейдите по ссылке:\n{$resetLink}\n\nСсылка действительна в течение 1 часа.");
