#!/usr/bin/env python3
"""Fix single test page routing and create template version"""
...
# Single test page using real_template.php
single_test_page = '''<?php
/**
...
htaccess_rules = '''
# Single test pages
RewriteRule ^test/([a-zA-Z0-9-]+)/?$ /pages/tests/test-single-real.php?url_test=$1 [L]
'''

try:
    print("Fixing single test page...")
    ftp = ftplib.FTP(FTP_HOST)
    ftp.login(FTP_USER, FTP_PASS)
    ftp.cwd(FTP_PATH)
    
    # Create the single test page
    with open('test-single-real.php', 'w') as f:
        f.write(single_test_page)
    
    with open('test-single-real.php', 'rb') as f:
        ftp.storbinary('STOR pages/tests/test-single-real.php', f)
    
    print("✓ Created single test page")
    
    # Also create a direct file for testing
    with open('test-single-real.php', 'rb') as f:
        ftp.storbinary('STOR test-single-real.php', f)
    
    print("✓ Created test file at root for debugging")
    
    ftp.quit()
    
    print("\n✅ Single test page fixed!")
    print("\nNow test these URLs:")
    print("- https://11klassniki.ru/test/russian-language-test")
    print("- https://11klassniki.ru/test/ege-math-test")
    print("- https://11klassniki.ru/test/physics-test")
    print("\nNote: You may need to update .htaccess with the routing rule:")
    print("RewriteRule ^test/([a-zA-Z0-9-]+)/?$ /pages/tests/test-single-real.php?url_test=$1 [L]")
