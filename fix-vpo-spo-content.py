#!/usr/bin/env python3
"""Fix VPO/SPO pages to show content with fallback data"""
...
# Updated educational institutions page with fallback content
...
try:
    print("Fixing VPO/SPO pages with content...")
    ftp = ftplib.FTP(FTP_HOST)
    ftp.login(FTP_USER, FTP_PASS)
    ftp.cwd(FTP_PATH)
    
    # Save and upload the fixed educational institutions page
    with open('educational-institutions-all-regions-real-fixed.php', 'w') as f:
        f.write(edu_page_content)
    
    with open('educational-institutions-all-regions-real-fixed.php', 'rb') as f:
        ftp.storbinary('STOR pages/common/educational-institutions-all-regions/educational-institutions-all-regions-real.php', f)
    
    print("✓ Fixed educational institutions page with fallback content")
    
    ftp.quit()
    
    print("\n✅ VPO/SPO/Schools pages fixed!")
    print("\nThe pages will now show:")
    print("1. Real data from database if available") 
    print("2. Sample/demo data if no real data found")
    print("3. Notice when showing demo data")
    print("\nTest these pages:")
