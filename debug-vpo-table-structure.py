...
<div class="container">
    <div class="alert alert-info">
        <h4>🔍 DEBUG: VPO Table Structure</h4>';

if ($connection) {
    // Show all column names
    $result = mysqli_query($connection, "DESCRIBE vpo");
    if ($result) {
        $greyContent5 .= '<p><strong>All columns in VPO table:</strong><br>';
        while ($row = mysqli_fetch_assoc($result)) {
            $greyContent5 .= $row['Field'] . ' (' . $row['Type'] . '), ';
        }
        $greyContent5 .= '</p>';
    }
    
    // Show sample data from first row
    $result = mysqli_query($connection, "SELECT * FROM vpo LIMIT 1");
    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $greyContent5 .= '<p><strong>Sample data from first record:</strong><br>';
        foreach ($row as $field => $value) {
            if (!empty($value)) {
                $greyContent5 .= $field . ': ' . htmlspecialchars(substr($value, 0, 50)) . '<br>';
            }
        }
        $greyContent5 .= '</p>';
    }
    ...
    $fields_to_test = ['region_id', 'town_id', 'area_id', 'country_id'];
    foreach ($fields_to_test as $field) {
        $result = mysqli_query($connection, "SELECT COUNT(DISTINCT $field) as c FROM vpo WHERE $field IS NOT NULL");
        if ($result) {
            $count = mysqli_fetch_assoc($result)['c'];
            $greyContent5 .= '<p>' . $field . ': ' . $count . ' distinct values</p>';
        }
    }
}
