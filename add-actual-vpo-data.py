...
// Get actual VPO data to display
if ($connection && $total_institutions > 0) {
    $result = mysqli_query($connection, "SELECT name, town FROM vpo WHERE name IS NOT NULL AND name != '' LIMIT 12");
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        <div class="col-12">
            <div class="alert alert-info">
                <p>Данные в базе есть (' . number_format($total_institutions) . ' записей), но поля name или town пустые.</p>
            </div>
        </div>';
    }
} else {
    ...
    <div class="col-12">
        <div class="alert alert-warning">
            <p>Не удалось загрузить примеры ВУЗов из базы данных.</p>
        </div>
    </div>';
}
...
if __name__ == "__main__":
    print("=== Adding Actual VPO Data Display ===")
    fix_vpo_page()