...
<head>
    ...
    <title>Управление постами - 11классники</title>
    <style>
        * { 
            margin: 0; 
            padding: 0; 
    ...
    <div class="sidebar" id="sidebar">
        ...
        <nav class="nav">
            ...
            <div class="nav-section">
                ...
                <a href="/dashboard/posts" class="nav-item active">
                    <span class="nav-icon">📋</span>
                    Управление постами
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="content">
            ...
            <div class="filter-tabs">
                <a href="/dashboard/posts?filter=all" class="filter-btn <?= $filter === 'all' ? 'active' : '' ?>">
                    📋 Все посты
                    <?php if ($posts_total > 0): ?>
                    <span style="background: rgba(37, 99, 235, 0.1); color: var(--primary); padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-left: 8px;"><?= $posts_total ?></span>
                    <?php endif; ?>
                </a>
                <a href="/dashboard/posts?filter=published" class="filter-btn <?= $filter === 'published' ? 'active' : '' ?>">
                    ✅ Опубликованные
                    <?php if ($posts_total > 0): ?>
                    <span style="background: rgba(16, 185, 129, 0.1); color: var(--success); padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-left: 8px;"><?= $posts_total ?></span>
                    <?php else: ?>
                    <span style="background: rgba(16, 185, 129, 0.1); color: var(--success); padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-left: 8px;">0</span>
                    <?php endif; ?>
                </a>
                <a href="/dashboard/posts?filter=draft" class="filter-btn <?= $filter === 'draft' ? 'active' : '' ?>">
                    📝 Черновики
                    <span style="background: rgba(245, 158, 11, 0.1); color: var(--warning); padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-left: 8px;">0</span>
                </a>
            </div>
            ...
            <div class="news-card">
                <div class="news-card-header">
                    ...
                    <a href="/create/post" class="btn-create">➕ Создать пост</a>
                </div>
                
                <div class="table-container">
                    <?php
                    // Posts don't have an approved field, so we'll check all posts
                    // In the future, you might want to add a status field to posts table
                    
                    // Query posts
                    $sql = "SELECT id_post, title_post, author_post, date_post, view_post, url_post 
                            FROM posts 
                    ...
                    if ($result && $result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            $date = $row['date_post'] ? date('d.m.Y', strtotime($row['date_post'])) : 'Не указана';
                            
                            echo '<tr>';
                            echo '<td>' . $row['id_post'] . '</td>';
                            echo '<td><strong>' . htmlspecialchars(mb_substr($row['title_post'], 0, 60)) . '...</strong></td>';
                            echo '<td>' . htmlspecialchars($row['author_post'] ?? 'Admin') . '</td>';
                            echo '<td>' . $date . '</td>';
                            echo '<td>' . ($row['view_post'] ?? 0) . '</td>';
                            echo '<td>';
                            echo '<div class="action-buttons">';
                            echo '<a href="/post/' . htmlspecialchars($row['url_post']) . '" class="action-btn action-edit">👁️ Просмотр</a>';
                            echo '<a href="/delete-post.php?id=' . $row['id_post'] . '" onclick="return confirm(\'Удалить этот пост?\')" class="action-btn action-delete">🗑️ Удалить</a>';
                            echo '</div>';
                            echo '</td>';
                            echo '</tr>';
                        }
