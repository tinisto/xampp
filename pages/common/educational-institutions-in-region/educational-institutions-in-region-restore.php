...
// Get institutions
$institutions_query = "SELECT * FROM $type WHERE id_region = ? LIMIT $pageOffset, $institutionsPerPage";
...
$total_query = "SELECT COUNT(*) AS total FROM $type WHERE id_region = ?";
                    ...
                    <?php if ($institutions_result && $institutions_result->num_rows > 0): ?>
                        <?php while ($institution = $institutions_result->fetch_assoc()): ?>
                            <div class="institution-card">
                                <h3>
                                    <?php if ($type === 'schools'): ?>
                                        <a href="/schools/<?= htmlspecialchars($region_name_en) ?>/<?= htmlspecialchars($institution['id_school']) ?>">
                                            <?= htmlspecialchars($institution['school_name'] ?? $institution['name'] ?? 'Название не указано') ?>
                                        </a>
                                    <?php else: ?>
                                        <?= htmlspecialchars($institution['name'] ?? $institution['institution_name'] ?? 'Название не указано') ?>
                                    <?php endif; ?>
                                </h3>
                                
                                <?php if (isset($institution['address']) && !empty($institution['address'])): ?>
                                    <p><i class="fas fa-map-marker-alt"></i> <?= htmlspecialchars($institution['address']) ?></p>
                                <?php endif; ?>
                                
                                <?php if (isset($institution['phone']) && !empty($institution['phone'])): ?>
                                    <p><i class="fas fa-phone"></i> <?= htmlspecialchars($institution['phone']) ?></p>
                                <?php endif; ?>
                                
                                <?php if (isset($institution['website']) && !empty($institution['website'])): ?>
                                    <p><i class="fas fa-globe"></i> <a href="<?= htmlspecialchars($institution['website']) ?>" target="_blank"><?= htmlspecialchars($institution['website']) ?></a></p>
                                <?php endif; ?>
                            </div>
                        <?php endwhile; ?>
