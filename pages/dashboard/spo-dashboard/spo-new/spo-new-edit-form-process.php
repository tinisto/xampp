...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $collegeId = filter_input(INPUT_POST, 'id_spo', FILTER_SANITIZE_NUMBER_INT);
    $approved = filter_input(INPUT_POST, 'approved', FILTER_SANITIZE_NUMBER_INT);

    // Sanitize fields for HTML output
    $collegeName = filter_input(INPUT_POST, 'spo_name', FILTER_SANITIZE_FULL_SPECIAL_CHARS); // For HTML display
    ...
    $url = filter_input(INPUT_POST, 'spo_url', FILTER_SANITIZE_URL); // URL sanitization
    ...
    $query = $connection->prepare("UPDATE spo SET
                                      spo_name = ?, approved = ?, name_rod = ?, full_name = ?,
                                      short_name = ?, old_name = ?, spo_url = ?, site = ?, email = ?,
                                      tel = ?, fax = ?, site_pk = ?, email_pk = ?, tel_pk = ?,
                                      otvetcek = ?, accreditation = ?, licence = ?, director_role = ?,
                                      director_name = ?, director_info = ?, director_phone = ?,
                                      director_email = ?, history = ? WHERE id_spo = ?");
    ...
    if ($result) {
        if ($approved == 1) {
            ...
            include_once $_SERVER['DOCUMENT_ROOT'] . '/includes/email-templates/spo-email-template-approve.php';
            sendToAdmin($subject, $body);
            sendToUser($emailUser, $subject, $body);
            header("Location: /spo/$url");
            exit();
        } else {
        ...
        error_log("Error updating spo: " . $query->error);  // Log the error to a file
        redirectToErrorPage($connection->error, __FILE__, __LINE__);
    }
