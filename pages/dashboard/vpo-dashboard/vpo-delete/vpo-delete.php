...
if (isset($_GET["id"])) {
    ...
    $id_vpo = filter_var($_GET["id"], FILTER_SANITIZE_NUMBER_INT);
    ...
    // Step 1: Delete all comments associated with the university
    // Delete child comments first (comments with parent_id)
    $deleteChildCommentsQuery = "DELETE FROM comments WHERE id_entity = ? AND entity_type = 'vpo' AND parent_id IS NOT NULL";
    ...
    $stmtDeleteChildComments->bind_param("i", $id_vpo);
    ...
    $deleteParentCommentsQuery = "DELETE FROM comments WHERE id_entity = ? AND entity_type = 'vpo' AND parent_id IS NULL";
    ...
    $stmtDeleteParentComments->bind_param("i", $id_vpo);
    if (!$stmtDeleteParentComments->execute()) {
        error_log("Error executing query for parent comments: " . $stmtDeleteParentComments->error);
        redirectToErrorPage($connection->error, __FILE__, __LINE__);
    }

    // Step 2: Delete the university itself
    $deleteQuery = "DELETE FROM vpo WHERE id_vpo = ?";
    $deleteStatement = $connection->prepare($deleteQuery);

    if ($deleteStatement === false) {
        // Log MySQL error if prepare() fails
        error_log("MySQL prepare error (deleting university): " . $connection->error);
        redirectToErrorPage($connection->error, __FILE__, __LINE__);
    }

    $deleteStatement->bind_param("i", $id_vpo);
    $result = $deleteStatement->execute();

    if (!$result) {
        error_log("Error deleting university: " . $deleteStatement->error);
        redirectToErrorPage($connection->error, __FILE__, __LINE__);
    }

    $_SESSION["success-message"] = "University and associated comments deleted successfully.";
    ...
    $_SESSION["error-message"] = "No id_vpo specified in the URL.";
