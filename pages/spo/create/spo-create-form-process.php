...
if ($_SESSION['occupation'] !== 'Представитель ССУЗа') {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $newCollegeName = $_POST['spo_name'];
    // Sanitize the name to remove unwanted characters
    $newCollegeName = preg_replace('/[^a-zA-Z0-9\s-]/', '', $newCollegeName);  // Remove non-alphanumeric characters except space and hyphen

    // Prepare the SQL query to prevent SQL injection
    $query = $connection->prepare("SELECT COUNT(*) AS count FROM spo WHERE spo_name = ?");
    ...
    $query->bind_param("s", $newCollegeName);
    ...
    if ($query->execute()) {
        ...
        if ($queryResult) {
            ...
            if ($row['count'] > 0) {
                ...
                header("Location: /spo-create-form.php");
                exit();
            }
        } else {
    ...
    $spo_url = makeUrlFriendly($spo_name);
    $user_id = $_SESSION['email'];
    $user_id = $_SESSION['user_id'];

    // Prepare the insert statement
    $query = $connection->prepare("INSERT INTO spo (
        approved, spo_name, full_name, short_name, name_rod, old_name, site, email, tel, fax, site_pk, email_pk, tel_pk, otvetcek, accreditation, licence, director_role, director_name, director_info, director_phone, director_email, history, spo_url, user_id, user_id
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($insertResult) {
        ...
        $bodyToAdmin = "spo_name: $spo_name";
        sendToAdmin("SPO created", $bodyToAdmin); // No need to pass adminEmail, it'll use the default
