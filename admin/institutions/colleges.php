...
if (!empty($search)) {
    $whereClause = "WHERE (name_college LIKE ? OR abbr_college LIKE ?)";
    $searchParam = '%' . $search . '%';
    $searchParams = [$searchParam, $searchParam];
}

// Get total count with prepared statement
$countQuery = "SELECT COUNT(*) as total FROM colleges $whereClause";
if (!empty($searchParams)) {
    $stmt = $connection->prepare($countQuery);
    $stmt->bind_param("ss", ...$searchParams);
    $stmt->execute();
    $result = $stmt->get_result();
    $totalSPO = $result->fetch_assoc()['total'];
    $stmt->close();
} else {
    $result = $connection->query($countQuery);
    $totalSPO = $result ? $result->fetch_assoc()['total'] : 0;
}
$totalPages = ceil($totalSPO / $limit);

// Get colleges with prepared statement
...
<div style="padding: 20px;">
    <div style="max-width: 1200px; margin: 0 auto;">
        <div style="display: flex; gap: 20px; align-items: center; justify-content: space-between; flex-wrap: wrap;">
            ...
            <div style="display: flex; gap: 10px;">
                <a href="/admin/institutions/colleges.php?action=new" class="action-btn primary">
                ...
                <tr style="border-bottom: 1px solid #dee2e6;">
                    ...
                    <td style="padding: 15px;">
                        <div style="display: flex; gap: 10px; justify-content: center;">
                            <a href="/admin/institutions/colleges.php?action=edit&id=<?= $college['id_college'] ?>" 
...
function deleteSPO(collegeId) {
    if (confirm('Вы уверены, что хотите удалить это СПО?')) {
        ...
        window.location.href = '/admin/institutions/colleges.php?action=delete&id=' + collegeId;
    }
}
</script>
