...
<div style="padding: 20px;">
    <div style="max-width: 1200px; margin: 0 auto;">
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 20px; flex-wrap: wrap;">
            <a href="/admin/content/comments.php" class="filter-tab <?= $filter === 'all' ? 'active' : '' ?>">
                Все (<?= $stats['total'] ?>)
            </a>
            <a href="/admin/content/comments.php?filter=approved" class="filter-tab <?= $filter === 'approved' ? 'active' : '' ?>">
                Одобренные (<?= $stats['approved'] ?? 0 ?>)
            </a>
            <a href="/admin/content/comments.php?filter=pending" class="filter-tab <?= $filter === 'pending' ? 'active' : '' ?>">
...
function approveComment(commentId) {
    if (confirm('Одобрить этот комментарий?')) {
        ...
        window.location.href = '/admin/content/comments.php?action=approve&id=' + commentId;
    }
}
...
function unapproveComment(commentId) {
    if (confirm('Снять одобрение с комментария?')) {
        ...
        window.location.href = '/admin/content/comments.php?action=unapprove&id=' + commentId;
    }
}
...
function deleteComment(commentId) {
    if (confirm('Вы уверены, что хотите удалить этот комментарий?')) {
        ...
        window.location.href = '/admin/content/comments.php?action=delete&id=' + commentId;
    }
}
</script>
