...
// Get regions with VPO counts using region_id (known structure)  
if ($connection && $total_institutions > 0) {
    $result = mysqli_query($connection, "SELECT region_id, COUNT(*) as count FROM vpo WHERE region_id IS NOT NULL GROUP BY region_id ORDER BY count DESC");
    
    if ($result && mysqli_num_rows($result) > 0) {
        // Russian regions mapping
        $regions = [
            1 => 'Москва', 2 => 'Санкт-Петербург', 3 => 'Московская область',
            4 => 'Новосибирская область', 5 => 'Свердловская область', 6 => 'Нижегородская область',
            7 => 'Республика Татарстан', 8 => 'Краснодарский край', 9 => 'Челябинская область',
            10 => 'Ростовская область', 11 => 'Самарская область', 12 => 'Омская область'
        ];
        
        while ($row = mysqli_fetch_assoc($result)) {
            $region_id = $row['region_id'];
            $count = $row['count'];
            $regionName = $regions[$region_id] ?? "Регион #$region_id";
            $regionUrl = strtolower(str_replace([' ', 'ё'], ['-', 'е'], $regionName));
                ...
                <a href="/vpo-in-region/' . htmlspecialchars($regionUrl) . '" class="text-decoration-none">
                    <div class="card h-100 shadow-sm border-0" style="transition: all 0.3s ease;">
                        <div class="card-body text-center">
                            <h5 class="card-title text-primary mb-2">' . htmlspecialchars($regionName) . '</h5>
                            <div class="badge bg-success fs-6">' . number_format($count) . ' ВУЗов</div>
                        </div>
                    </div>
                </a>
            </div>';
        ...
        $greyContent5 .= '<div class="col-12"><div class="alert alert-info">Группировка по регионам обрабатывается...</div></div>';
    }
} else {
    $greyContent5 .= '<div class="col-12"><div class="alert alert-warning">Нет подключения к базе данных.</div></div>';
}
