<?php
// Colleges (SPO) management dashboard - Functional version
...
$totalColleges = $countResult ? $countResult->fetch_assoc()['total'] : 0;
...
// Get statistics
$stats = [];
$stats['total'] = $totalColleges;

// Colleges by region
$regionQuery = "SELECT region, COUNT(*) as count FROM colleges GROUP BY region ORDER BY count DESC LIMIT 5";
...
// Recent colleges (last 30 days)
$recentQuery = "SELECT COUNT(*) as count FROM colleges WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
...
<div class="spo-header">
    <h2>Управление СПО</h2>
    <a href="/create/spo" class="btn-primary">
...
<form method="GET" class="search-box">
    <input type="text" name="search" placeholder="Поиск по названию колледжа, городу или региону..." 
           value="<?= htmlspecialchars($search) ?>" class="search-input">
    <button type="submit" class="btn-search">
        <i class="fas fa-search"></i> Поиск
    </button>
    <?php if ($search): ?>
        <a href="/dashboard/spo" class="btn-search" style="background: #6c757d;">
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div class="stat-label">Всего СПО</div>
    </div>
    <div class="stat-card">
        ...
        <tbody>
            <?php if (!empty($colleges)): ?>
                ...
                <tr>
                    ...
                    <td>
                        <div class="college-name"><?= htmlspecialchars($college['name']) ?></div>
                    </td>
                    <td><?= htmlspecialchars($college['city'] ?? 'Не указан') ?></td>
                    <td><?= htmlspecialchars($college['region'] ?? 'Не указан') ?></td>
                    <td><?= $college['created_at'] ? date('d.m.Y', strtotime($college['created_at'])) : 'Не указана' ?></td>
                    <td>
                        <div class="action-buttons">
                            <a href="/spo/<?= $college['id'] ?>" class="btn-sm btn-view" target="_blank" title="Просмотр">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="/edit/spo/<?= $college['id'] ?>" class="btn-sm btn-edit" title="Редактировать">
...
<div>
    ...
    renderPaginationModern($page, $totalPages, '/dashboard/spo');
    ?>
</div>
<?php endif; ?>
    ...
    <div class="region-list">
        <?php foreach ($topRegions as $region): ?>
            <div class="region-item">
                ...
                <span class="region-count"><?= $region['count'] ?> СПО</span>
            </div>
        <?php endforeach; ?>
    </div>
</div>
