<?php
// VPO all regions page - migrated to use real_template.php
require_once $_SERVER['DOCUMENT_ROOT'] . '/database/db_connections.php';

// Type is always 'vpo' for this file
$type = 'vpo';
$table = 'vpo';
...
renderRealTitle($pageTitle, [
    'fontSize' => '36px',
    'margin' => '30px 0',
    'subtitle' => 'Выберите регион для просмотра высших учебных заведений'
]);
...
<div style="display: flex; justify-content: space-around; flex-wrap: wrap; gap: 20px; padding: 20px;">
    ...
    $totalVPO = mysqli_fetch_assoc($totalResult)['total'];
    
    // Get regions count
    $regionsQuery = "SELECT COUNT(DISTINCT $regionColumn) as total FROM $table WHERE $regionColumn IS NOT NULL";
    $regionsResult = mysqli_query($connection, $regionsQuery);
    $totalRegions = mysqli_fetch_assoc($regionsResult)['total'];
    ?>
    <div style="text-align: center;">
        <div style="font-size: 32px; font-weight: 700; color: #28a745;"><?= number_format($totalVPO) ?></div>
        <div style="font-size: 16px; color: #666;">Всего ВУЗов</div>
    </div>
    <div style="text-align: center;">
...
ob_start();
// Get regions data with VPO counts
$query = "SELECT 
            r.id_region, 
            r.title_region, 
            r.url_region,
            ...
            <div style="background: #f8f9fa; border: 1px solid #ddd; border-radius: 8px; padding: 20px; transition: all 0.3s ease; cursor: pointer;"
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        ...
                        <span style="color: #666; font-size: 14px;">ВУЗов</span>
                    </div>
