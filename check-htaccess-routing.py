...
?>'''
    ...
    try:
        ...
        # Check .htaccess file first
        try:
            print("Downloading .htaccess to check routing...")
            with open('htaccess_check.txt', 'wb') as f:
                ftp.retrbinary('RETR .htaccess', f.write)
            
            # Read and analyze .htaccess
            with open('htaccess_check.txt', 'r') as f:
                htaccess_content = f.read()
            
            # Find VPO routing rules
            lines = htaccess_content.split('\\n')
            vpo_rules = [line for line in lines if 'vpo-all-regions' in line.lower()]
            
            print("Found VPO routing rules:")
            for rule in vpo_rules:
                print(f"  {rule}")
                
            import os
            os.remove('htaccess_check.txt')
            
        except Exception as e:
            print(f"Could not check .htaccess: {e}")
        
        # Upload debug to discovered and common files
        possible_files = [
            ...
            'pages/vpo/vpo-all-regions.php',
            'pages/common/educational-institutions/vpo-all-regions.php'
        ]
